...
class FE:
    ...
    def __prep__(self):
        ...
        ##If file is pcap
        elif type_ == "pcap" or type_ == 'pcapng':
    ...
    def get_next_vector(self):
        ...
        if self.parse_type == "tsv":
            ...
            #print(row)
            IPtype = np.nan
            timestamp = row[0]#frame.time_epoch
            framelen = row[1]
            srcIP = ''
            dstIP = ''
            protocol=row[20]
            if row[4] != '':  # IPv4 #SrcAddr
                srcIP = row[4]
                dstIP = row[5]
                IPtype = 0
            elif row[17] != '':  # ipv6 ipv6.src 
                srcIP = row[17]
                dstIP = row[18]
                IPtype = 1
            srcproto = row[6] + row[8]  # tcp.srcport udp.srcport
        ...
        #print(framelen,int(framelen))

        ### Extract Features
        #print(IPtype, srcMAC, dstMAC, srcIP, srcproto, dstIP, dstproto,int(framelen),float(timestamp))
        try:
            return self.nstat.updateGetStats(IPtype, srcMAC, dstMAC, srcIP, srcproto, dstIP, dstproto,
                                                 int(framelen),
                                                 float(timestamp),protocol)
